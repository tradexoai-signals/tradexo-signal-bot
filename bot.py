...
# Free gets basic info, paid gets full details
if status_type == “new”:
# Free channel - limited info
free_msg = (
...
time.sleep(0.3)
# Paid channels get full signal
for plan in [“starter”,“pro”,“vip”]:
send_telegram(channels[plan], msg, cache_key=“new_”+plan+”:”+coin_k)
time.sleep(0.2)
else:
# Result notifications to all channels
# Result notifications to all channels with cache
...
```
# 5m confirmation — EMA9/EMA21 cross check
...
# SIGNAL EXPIRY — auto close expired signals
...
try:
r = requests.get(SUPA+”/rest/v1/bot_signals?status=eq.ACTIVE&select=*”,
...
headers=HD,
json={“status”:new_status,“exit_price”:round(curr,6),“pnl_pct”:pnl_pct},
timeout=10)
log.info(”  %s %s -> %s | PnL: %s%%”, coin, direction, new_status,
pnl_pct if pnl_pct is not None else “N/A”)
# Telegram notification
updated_sig = dict(sig)
updated_sig[“exit_price”] = round(curr,6)
updated_sig[“pnl_pct”] = pnl_pct
notify_all_channels(updated_sig, “tp1” if “TP” in new_status else “sl”)
time.sleep(0.2)
except Exception as e:
