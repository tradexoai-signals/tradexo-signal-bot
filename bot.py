...
if bull >= MIN_CONFIDENCE and bull > bear: direction,conf = ‘LONG’,min(bull,95)
elif bear >= MIN_CONFIDENCE and bear > bull: direction,conf = ‘SHORT’,min(bear,95)
else: return None
dec = 2 if price > 100 else (4 if price > 1 else 6)
if direction == ‘LONG’:
sl_p = price-(atr_v*1.5); tp1 = price+(atr_v*2); tp2 = price+(atr_v*3.5)
else:
sl_p = price+(atr_v*1.5); tp1 = price-(atr_v*2); tp2 = price-(atr_v*3.5)
risk = abs(price-sl_p)
...
time.sleep(0.2)
if sig:
print(f”  -> {sig[‘direction’]} {sig[‘confidence’]}%”)
new_sigs.append(sig)

print(f”Signals found: {len(new_sigs)}”)
for sig in new_sigs:
post_signal(sig)
